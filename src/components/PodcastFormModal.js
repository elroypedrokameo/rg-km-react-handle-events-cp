...
const PodcastFormModal = (props) => {
  ...
  const API_URL = `https://62694140f2c0cdabac0bc516.mockapi.io/podcasts/api/Podcasts`
  ...
  const handleFormSubmit = async () => {
    ...
    if (formModalType === "ADD") {
      const responseAdd = await axios.post(API_URL)
      setPodcastList([...podcastList, responseAdd])
    }

    if (formModalType === "UPDATE") {
      const responseUpdate = await axios.put(`${API_URL}/${podcastId}`, formValues)
